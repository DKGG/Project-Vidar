...
public class cameraOrbit : MonoBehaviour
{
    ...
    void Start()
    {
        _XTForm_camera = transform;
        _XTForm_parent = transform.parent;
    }
    ...
    void LateUpdate()
    {
        ...
        if(Input.GetAxis("Mouse ScrollWheel") != 0f)
        {
            ...
            _cameraDistance = Mathf.Clamp(_cameraDistance, 3f,10f);                
        }
        
        //orientações da camera
        Quaternion QT = Quaternion.Euler(localRotation.y, localRotation.x, 0);
        _XTForm_parent.rotation = Quaternion.Lerp(_XTForm_parent.rotation, QT, 0.1f* orbitSpeed);
        if (_XTForm_camera.localPosition.z != _cameraDistance * -1f)
        {
            _XTForm_camera.localPosition = new Vector3(0f, 0f, Mathf.Lerp(_XTForm_camera.localPosition.z, _cameraDistance * -1f,0.1f*scrollSpeed));
        }
